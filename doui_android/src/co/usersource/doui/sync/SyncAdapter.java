...
public class SyncAdapter extends AbstractThreadedSyncAdapter {
	...
	private JSONObject getLocalData(String strFieldName, Uri uri, String strType) {
		...
		answer = getContext().getContentResolver().query(uri, null, selection,
				null, null);
		...
		answer = getContext().getContentResolver().query(
				...
				null);
		updatedObjects = createJSONData(answer,
				SyncAdapter.JSON_UPDATED_TYPE_CATEGORIES, updatedObjects);
		// /////////////////////////////////////////////////////////////////////////////////////////
		...
		answer = getContext().getContentResolver()
				.query(DouiContentProvider.TODO_ITEMS_URI, null, selection,
						null, null);
		updatedObjects = createJSONData(answer,
				SyncAdapter.JSON_UPDATED_TYPE_ITEMS, updatedObjects);
		// /////////////////////////////////////////////////////////////////////////////////////////
		...
		try {
			if (mLastUpdateDate == null) {
				...
				request.put(SyncAdapter.JSON_LAST_UPDATE_TIMESTAMP,
						mLastUpdateDate);
			}
		...
		if (data != null) {
			...
			try {
				...
				for (int i = 0; i < dataFromServer.length(); ++i) {
					...
					currentValues = dataFromServer.getJSONObject(i)
					...
					for (int j = 0; j < currentValues.length(); ++j) {
						if (dataFromServer
							...
							if (currentValues.getJSONObject(j)
									.getString("client_id").equals("null")) {
								selection = TableTodoCategoriesAdapter.TABLE_TODO_CATEGORIES_OBJECT_KEY
										...
										+ currentValues
												...
												.getString(
														JSON_UPDATED_OBJECT_KEY)
										+ "'";
								localData = getContext()
										.getContentResolver()
										.query(DouiContentProvider.TODO_CATEGORIES_URI,
								...
								if (localData != null
									...
									if (getContext()
											.getContentResolver()
											.insert(DouiContentProvider.TODO_CATEGORIES_URI,
								...
								uriForUpdate = Uri
										.parse(DouiContentProvider.TODO_CATEGORIES_URI
												...
												+ currentValues
														...
														.getString("client_id"));
								getContext().getContentResolver().update(
										uriForUpdate, m_valuesForUpdate, null,
										null);
							}
						}
						...
						if (dataFromServer.getJSONObject(i)
							...
							if (!currentValues
									...
									.equals("null")) {
								int nCategoryId = getCategoryIDByObjectKey(currentValues
										.getJSONObject(j)
										.getString(
												TableTodoItemsAdapter.TABLE_TODO_ITEMS_FK_CATEGORY));
								if (nCategoryId != -1) {
							...
							if (!currentValues
									...
									.equals("null")) {
								int nStatusId = getStatusIDByObjectKey(currentValues
										.getJSONObject(j)
										.getString(
												TableTodoItemsAdapter.TABLE_TODO_ITEMS_FK_STATUS));
								if (nStatusId != -1) {
							...
							if (currentValues.getJSONObject(j)
								...
								localData = getContext()
										.getContentResolver()
										.query(DouiContentProvider.TODO_ITEMS_URI,
												null, selection, null, null);
								if (localData != null
										&& localData.moveToFirst()) {
									uriForUpdate = Uri
